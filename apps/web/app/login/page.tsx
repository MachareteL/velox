    ...
    try {
      if (isSignUp) {
        ...
        if (data.session) {
          ...
          setSuccessMsg('Cadastro realizado! Se necessário, confirme o link enviado ao seu e-mail.');
        }
      } else {
        // Login de prestador existente
    ...
    <div className="min-h-screen bg-[#090d16] flex items-center justify-center p-4">
      <div className="max-w-md w-full glass-panel p-8 rounded-3xl border border-gray-800 shadow-2xl relative">
        <div className="text-center mb-8">
          ...
          <p className="text-xs text-gray-400 mt-1">Acesso exclusivo para prestadores de serviços</p>
        </div>

        {/* Tab Selector */}
        <div className="flex bg-gray-950/80 p-1 rounded-xl border border-gray-800 mb-6">
          <button
            type="button"
            onClick={() => { setIsSignUp(false); setErrorMsg(null); }}
            className={`flex-1 py-2 text-xs font-semibold rounded-lg transition-all ${
              !isSignUp ? 'bg-emerald-600 text-white shadow-md' : 'text-gray-400 hover:text-white'
            }`}
          >
            Entrar
          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {isSignUp && (
            <div>
              <label className="block text-xs font-semibold text-gray-300 mb-1">Nome do Prestador / Empresa</label>
              <div className="relative">
                <UserIcon className="w-4 h-4 text-gray-500 absolute left-3.5 top-3" />
                <input
                  type="text"
                  required
                  placeholder="Ex: João Guincho 24h"
                  value={name}
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-300 mb-1">E-mail Profissional</label>
            <div className="relative">
          ...
          {isSignUp && (
            <div>
              <label className="block text-xs font-semibold text-gray-300 mb-1">Código de Licença / Acesso (Opcional)</label>
              <div className="relative">
                <ShieldCheck className="w-4 h-4 text-gray-500 absolute left-3.5 top-3" />
                <input
                  type="text"
                  placeholder="Ex: VELOX-VIP-2026"
                  value={accessCode}
